...
function getFibonacciSequence(size: number): number[] {
  ...
  for (let i = 2; i < size; i++) {
    const dernierchiffre = fibonacciSequence[fibonacciSequence.length - 1];
    const avantdernierchiffre = fibonacciSequence[fibonacciSequence.length - 2];
    const prochainchiffre = dernierchiffre + avantdernierchiffre;
    fibonacciSequence.push(prochainchiffre);
  }
